...
<link rel="stylesheet" href="/resources/css/garden.css" type="text/css">	
</head>
<body>
...
<table class="table" width="100%" style="border: 1px solid #dee2e6;" cellSpacing="0" cellPadding="0">
	<colgroup>
		<col width="20%"/>
		<col width="80%"/>
	</colgroup>
	<tr>
		...
		<td>
			<input type="text" class="form-control" style="width: 30%; float: left;" name="sText" value="<%=request.getParameter("sText")==null?"":request.getParameter("sText")%>">
			<input type="button" class="btn btn-success" style="float: left;" name="search" value="검색" onclick="return fncSearch();"/>
		</td>
	</tr>
	<tr>
		<th>
			<select  id="wordType" name="wordType" class="form-control" onchange="javascript:fncWordTypeOption(); return false;">
...
<%
	...
	String apiKey="20200720XOLVXHEUOZ7UB1XBTJ8OW";
	//서비스 명
...
%>
				...
				<select class="form-control" style="width: 40%" id="priceTypeSel" name="priceTypeSel">
	...
	if(operationNameMap.containsKey("waterCycleList")){
		...
		if(resultCode.equals("00")){
			for(int i=0; i<size; i++){
				...
				if(i == 0){
%>
				<select id="waterCycleSel" name="waterCycleSel" style="width: 40%" class="form-control">
				<option value="">선택하세요.</option>
<%
				}
%>
...
<%
	...
	if(operationName.length == operationNameMap.size()){
		...
		if(resultCode.equals("00")){ %>
		<hr>
		<table width="100%"  cellSpacing="0" cellPadding="0" class="table" style="border: 1px solid #dee2e6;">
<%
		if(size==0){
%>
			<tr>
...
<%--  					<a href="javascript:fncDtl('<%=cntntsNo%>');"><%=cntntsSj%></a> --%>
 					<a class="plantname" href="/plant.do?searchword=<%=cntntsSj%>"><%=cntntsSj%></a>
			    </td>
			</tr>
<%		
			}
	...
	//페이징 처리
		...
		if(count > 0){
			...
			if(numPageGroup > 1){
				...
				out.println("<a role='button' class='btn btn-success' href='javascript:fncGoPage("+prtPageNo+");'>이전</a>");
			}
			
			for(int i=startPage; i<=endPage; i++){
				prtPageNo = i;
				out.print("<a role='button' class='btn btn-success' href='javascript:fncGoPage("+prtPageNo+");'>");
				if(currentPage == i){
			...
			if(numPageGroup < pageGroupCount){
				...
				out.println("<a role='button' class='btn btn-success' href='javascript:fncGoPage("+prtPageNo+");'>다음</a>");
			}
		}
		//페이징 처리 끝
		}else{
