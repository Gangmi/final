...
<body>
	...
	<section class="hero-section">
		<div class="plantdiv">
			...
			<div class="tabdiv">
				<div class="tab">
					<a href="plant.do?plantcate=겉씨식물" class="btn"
						onclick="openCity(event, 'gymnosperms')" role="button">겉씨식물</a> <a
						href="plant.do?plantcate=속씨식물" class="btn"
						onclick="openCity(event, 'Angiospermae')" role="button">속씨식물</a> <a
						href="plant.do?plantcate=양치식물" class="btn"
						onclick="openCity(event, 'Pteridophyta')" role="button">양치식물</a> <a
						href="plant.do?plantcate=선태식물" class="btn"
						onclick="openCity(event, 'Bryophyte')" role="button">선태식물</a> <a
						href="plant.do?plantcate=조류" class="btn"
						onclick="openCity(event, 'algae')" role="button">조류</a> <a
						href="plant.do?plantcate=군류" class="btn"
						onclick="openCity(event, 'gunru')" role="button">군류</a> <a
						href="plant.do?plantcate=식물기타" class="btn"
						onclick="openCity(event, 'Plantguitar')" role="button">식물기타</a>
				</div>
			</div>
			<!-- 식물이미지,식물명,카테고리를 보여줌 -->
		...
		<div class="pages">
			<div class="page" id="pager">
				...
				<a class="btn btn-info"
					href="plant.do?nowpage=<%=nowstartpage - 1%><%if (request.getAttribute("searchword") != null) {%>
						&searchword=<%=(String) request.getAttribute("searchword")%><%}%>
						<%if (request.getAttribute("plantcate") != null) {%>&plantcate=<%=(String) request.getAttribute("plantcate")%><%}%>"
				...
				<%
					...
					%>
				<a class="btn btn-info"
					href="plant.do?nowpage=<%=i%><%if (request.getAttribute("searchword") != null) {%>
						&searchword=<%=(String) request.getAttribute("searchword")%><%}%>
						<%if (request.getAttribute("plantcate") != null) {%>&plantcate=<%=(String) request.getAttribute("plantcate")%><%}%>"
				...
				<%
					...
					%>
				<a class="btn btn-info"
					href="plant.do?nowpage=<%=nowlastpage + 1%><%if (request.getAttribute("searchword") != null) {%>
						&searchword=<%=(String) request.getAttribute("searchword")%><%}%>
						<%if (request.getAttribute("plantcate") != null) {%>&plantcate=<%=(String) request.getAttribute("plantcate")%><%}%>"
					id="pages" role="button">></a>
				<%
					...
					%>
				<a class="btn btn-info"
					href="plant.do?nowpage=<%=result.get(0).getNowpage()%><%if (request.getAttribute("searchword") != null) {%>
						&searchword=<%=(String) request.getAttribute("searchword")%><%}%>
						<%if (request.getAttribute("plantcate") != null) {%>&plantcate=<%=(String) request.getAttribute("plantcate")%><%}%>"
					id="pages" role="button">>></a>
				<%
