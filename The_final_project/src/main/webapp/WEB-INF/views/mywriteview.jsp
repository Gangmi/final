...
<body>
	...
	<section class="hero-section">
		<div class="container">
				...
				<table class="table table-hover table-condensed" id="boardtable">
					...
					%> 
						<tr>
							...
							<td width="659" class="titles"><a href="mywriteview.do?b_boardname=<%=boardname%>&boardno=<%=vo.getBoardno()%>&nickname=<%=vo.getNickname()%>"><%=vo.getTitle()%></a></td>
			...
			<!--만약 블록이 1보다 크다면  -->
			<%if(nowblock>1){ %>
			<a class="btn btn-info" href="mywriteview.do?b_boardname=<%=boardname%>&nowpage=<%=nowstartpage-1%><%if(request.getAttribute("searchword")!=null){ %>&searchword=<%=(String)request.getAttribute("searchword")%><%}%>" id="pages" role="button">이전으로</a>
			<%} %>	
			...
			%>
				...
				<a class="btn btn-info" href="mywriteview.do?b_boardname=<%=boardname%>&nowpage=<%=i%><%if(request.getAttribute("searchword")!=null){ %>&searchword=<%=(String)request.getAttribute("searchword")%><%}%>" id="pages" role="button"><%=i%></a>
			<%
			...
			<a class="btn btn-info" href="wmywriteview.do?b_boardname=<%=boardname%>&nowpage=<%=nowlastpage+1%><%if(request.getAttribute("searchword")!=null){ %>&searchword=<%=(String)request.getAttribute("searchword")%><%}%>" id="pages" role="button">다음으로</a>
			<%} %>
			
			<a class="btn btn-info" href="mywriteview.do?b_boardname=<%=boardname%>&nowpage=<%=result.get(0).getNowpage()%><%if(request.getAttribute("searchword")!=null){ %>&searchword=<%=(String)request.getAttribute("searchword")%><%}%>" id="pages" role="button">끝으로</a>
			<% 	
				}
			%>
			</div> 
