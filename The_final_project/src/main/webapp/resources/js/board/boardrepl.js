			...
			success : function(data){
				if(data.length>0){
					...
					for(i=0; i<data.length; i++){
						...
						if(userid != null){html+=" <button type='button' id=modify"+data[i].replno+" class='modifyRepl btn btn-cyan btn-sm'>수정</button> <button type='button' id=del"+data[i].replno+" class='delRepl btn btn-cyan btn-sm'>삭제</button></div>"};
						html+="						</div>"
						html+="                </div>";
						html+="            </div> <!-- Comment Row --> ";    
						html+="        </div> <!-- Card -->";	
			...
			success : function(data){
				...
				if(data.length>0){
					...
					for(i=0; i<data.length; i++){
						...
						html+="                    <div class='comment-footer'> <span class='text-muted float-right'>"+data[i].regdate+"</span> ";
						//접속한 유저와 같아야 수정 취소 버튼이 나온다.	
						if(userid != null){html+=" <button type='button' id=modify"+data[i].replno+" class='modifyRepl btn btn-cyan btn-sm'>수정</button> <button type='button' id=del"+data[i].replno+" class='delRepl btn btn-cyan btn-sm'>삭제</button></div>"}else{};
						html+="						</div>"
						html+="                </div>";
						html+="            </div> <!-- Comment Row --> ";    
						html+="        </div> <!-- Card -->";	
